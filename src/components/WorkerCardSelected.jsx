  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const options = {
          method: "GET",
          url: `https://dinmaegler.onrender.com/agents?_limit=3`,
        };
        console.log(options);
    ...
    <>
      {response.map((data, index) => (
        <StyledWorkerCard key={index} onClick={scrollToTop}>
          <Link to={`/AgentDetailsPage/${data.id}`} key={data.id}>
            <ImgFrame>
...
export default WorkerCardSelected;
